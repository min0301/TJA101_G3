...
public interface ProductRepository extends JpaRepository<Product, Integer>{
		...
		@Modifying
	    @Query("UPDATE Product p SET p.PRO_ISMARKET = :proIsMarket WHERE p.id = :proNo")
	    Integer updateMarketStatus(@Param("proNo") Integer proNo, @Param("proIsmarket") Character proIsMarket);
		
	    @Query("SELECT * FROM Product p WHERE p.MALL_TAG_NO = :malltagNo "+"AND (p.PRO_ISMARKET IS NULL OR p.PRO_ISMARKET = :proIsMarket)")
