	...
	const memberViews = {
		...
		collections: async function(member) {
			let html = `
	    		    <div class="member-center-card mb-4 bg-white">
	    		      <div class="card-body py-4 px-4">
	    		        <div class="card-section-title">關注討論區</div>
	    		        <div class="table-responsive">
	    		          <table class="table align-middle mb-0">
	    		            <thead>
	    		              <tr>
				...
				if (collections && collections.length > 0) {
					rows = collections.map(item => `
	    		        <tr>
	    		          ...
	    		          <td>${item.postUpdate ? item.pcollUpdate.replace('T', ' ').slice(0, 10) : '-'}</td>
	    		        </tr>
	    		      `).join('');
				} else {
					rows = `<tr><td colspan="2" class="text-center text-muted">您尚未關注任何討論區</td></tr>`;
				}
